...
impl Plugin for SpringPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(PreUpdate, update_springs);
    }   
}

fn update_springs(
    mut spring_query: Query<(&mut Spring, &mut Transform), Without<SpringUpdateIgnore>>,
    dt: Res<Time>
) {
    for (mut spring, mut transform) in spring_query.iter_mut() {
        spring.update(dt.delta_seconds());
        transform.translation = spring.target.extend(0.) + spring.velocity.extend(0.);
    }
}
...
impl Spring {
    pub fn new(target: Vec2, angular_freq: f32, damping_ratio: f32) -> Spring {
